...
def main(c1, c2, c3, c4, c5, c6, c7):
    ...
    fill[c6[c6last]].fill(TURQ)
    fill[c6[0]].fill(TURQ)

    fill[c7[c7last]].fill(LIGHTY)
    fill[c7[0]].fill(LIGHTY)
    ...
    # Booleans to check if first check was successful because if we increment wincount at that time, it depends on the speed of processing and user input
    redcomp = False
    bluecomp = False
    greencomp = False
    orangecomp = False
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                elif color[x, y] == DISPLAYSURF.map_rgb(TURQ):
                    isDragging = 1
                    choose = TURQ
                    del color

                elif color[x, y] == DISPLAYSURF.map_rgb(LIGHTY):
                    isDragging = 1
                    choose = LIGHTY
                    del color
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if b1.rect.collidepoint(x, y):
                    ...
                    while turqcount > 0:
                        fill[arr_turq[turqcount-1]].fill(BLACK)
                        arr_turq.pop(-1)
                        turqcount -= 1


                    while lightycount > 0:
                        fill[arr_lighty[lightycount-1]].fill(BLACK)
                        arr_lighty.pop(-1)
                        lightycount -= 1
            ...
            if event.type == pygame.MOUSEMOTION and isDragging == 1:
                ...
                elif color[x, y] == DISPLAYSURF.map_rgb(TURQ) and not choose == TURQ:
                    pygame.mouse.set_pos(prevx, prevy)

                elif color[x, y] == DISPLAYSURF.map_rgb(LIGHTY) and not choose == LIGHTY:
                    pygame.mouse.set_pos(prevx, prevy)
        ...
        tot = arr_red.__len__() + arr_green.__len__() + arr_blue.__len__() + arr_orange.__len__() + arr_purple.__len__() + arr_turq.__len__() + arr_lighty.__len__()
        if tot == n*n - n*2:
            ...
            if arr_orange.__len__() == c5.__len__() - 2 and not flagorange:
                ...
                for i in range(0, arr_orange.__len__()):
            ...
            if arr_turq.__len__() == c6.__len__() - 2 and not flagturq:

                for i in range(0, arr_turq.__len__()):
                    if (arr_turq[i] == c6[i+1]) or (arr_turq[i] == c6[(c6last-1)-i]):

                        if i == arr_turq.__len__() - 1:
                            flagturq = True
                            win_count += 1
                            print win_count
                        continue
            ...
            if arr_lighty.__len__() == c7.__len__() - 2 and not flaglighty:

                for i in range(0, arr_lighty.__len__()):
                    if (arr_lighty[i] == c7[i+1]) or (arr_lighty[i] == c7[(c7last-1)-i]):

                        if i == arr_lighty.__len__() - 1:
                            flaglighty = True
                            win_count += 1
                            print win_count
                        continue
